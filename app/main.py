...
def get_db() -> Generator[Session, None, None]:
    """
    Зависимость FastAPI для выдачи сессии БД с гарантированным закрытием.
    """
    db = session_local()
    try:
        yield db
...
def ui_requests_list(
    ...
    if q_clean:
        if q_clean.isdigit():
            ...
            query = query.filter(
                models.RepairRequest.problem_description.ilike(
                    f"%{q_clean}%",
                ),
            )
...
def ui_request_view(
    ...
    context = {
        ...
        "req": req,
        "comments": comments,
        "can_edit": rbac.user_can_edit_request(
            user=user,
            req=req,
        ),
        "can_add_comment": rbac.user_can_add_comment(
            user=user,
            req=req,
        ),
...
def ui_user_new(
    ...
    user = current_user_optional(request=request, db=db)
    if not user or not rbac.user_can_manage_users(user):
        return RedirectResponse(
            url="/ui/login?status=login_required"
            if not user
            else "/ui/requests?status=forbidden",
            status_code=status.HTTP_303_SEE_OTHER,
        )

    roles = (
        db.query(models.UserRole)
        .order_by(models.UserRole.name)
        .all()
    )
...
def ui_user_edit(
    ...
    if not target:
        return RedirectResponse(
            url="/ui/users?status=user_not_found",
            status_code=status.HTTP_303_SEE_OTHER,
        )
...
def ui_user_save(
    ...
    try:
        ...
        db.rollback()
        return RedirectResponse(
            url="/ui/users?status=db_error",
            status_code=status.HTTP_303_SEE_OTHER,
        )

    code = "user_updated" if is_edit else "user_created"
    return RedirectResponse(
        url=f"/ui/users?status={code}",
        status_code=status.HTTP_303_SEE_OTHER,
    )
...
def ui_user_delete(
    ...
    try:
        ...
        db.rollback()
        return RedirectResponse(
            url="/ui/users?status=user_delete_failed",
            status_code=status.HTTP_303_SEE_OTHER,
        )

    return RedirectResponse(
        url="/ui/users?status=user_deleted",
        status_code=status.HTTP_303_SEE_OTHER,
    )
...
def ui_request_qr(
    ...
    """
    Генерация QR-кода с ссылкой на форму оценки качества.
    QR-код используется в карточке заявки.
    """
    user = current_user_optional(request=request, db=db)
    if not user:
        return RedirectResponse(
