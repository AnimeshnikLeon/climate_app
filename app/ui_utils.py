...
def parse_int(value: Optional[str]) -> Optional[int]:
    ...
    Пустые строки и некорректные значения возвращают None.
    """
    if value is None:
        return None
...
def parse_date(
    ...
    """
    Унифицированный парсер дат из формы.
    В случае ошибки заполняет field_errors[field_key].
    """
...
def build_status_messages(request: Request) -> List[Dict[str, Any]]:
    """
    Преобразует параметр ?status=... в список сообщений для интерфейса.
    Каждое сообщение содержит тип, заголовок и текст.
    """
    ...
    mapping = {
        ...
        "comment_added": (
            ...
            "Сообщение специалиста сохранено.",
        ),
        "comment_empty": (
            "error",
