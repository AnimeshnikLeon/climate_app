...
class DomainError(Exception):
    ...
    Атрибут code используется для маппинга в статус UI (?status=...).
    """
...
class RequestInput:
    """
    Входные данные для создания/обновления заявки.
    """
...
def _apply_due_date(
    ...
    """
    Применяет плановый срок выполнения.
    Установить/изменить срок могут:
    - Менеджер
    - Оператор
...
def save_request(
    ...
    """
    Создание или обновление заявки с учётом бизнес‑логики и RBAC.
    Не выполняет commit — это обязанность вызывающего кода.
    """
...
def add_comment(
    ...
    if not rbac.user_can_add_comment(user=user, req=req):
        raise PermissionDeniedError(
            "Комментарий может добавлять только специалист по своей заявке.",
        )
